param([switch]$verbose, [switch]$ConsoleMode)
...
$script:IsHeadlessConsole = ($ConsoleMode -or $AutoOptimize)

if ($script:IsHeadlessConsole) {
...
function Create-ReservedCpuSetsUI {
    ...
    $btnSetReserved.Add_Click({
        ...
        try {
            ...
            if ($script:IsHeadlessConsole) {
                Write-Host '[ReservedCpuSets] Updated successfully.' -ForegroundColor Green
            } else {
            ...
            if ($script:IsHeadlessConsole) {
                Write-Host "[ReservedCpuSets] Failed: $_" -ForegroundColor Red
            } else {
...
if (-not $script:IsHeadlessConsole) {
    Write-Host "[AutoOpt] Verbose console mode." -ForegroundColor Cyan
}
...
$btnIRQ.Add_Click({
    ...
    try {
        ...
        if ($script:IsHeadlessConsole) {
            Write-Host '[IRQ] Counts calculated successfully.' -ForegroundColor Green
        } else {
        ...
        if ($script:IsHeadlessConsole) {
            Write-Host "[IRQ] Error calculating counts: $_" -ForegroundColor Red
        } else {
...
$imodPanel.BackColor = [System.Drawing.Color]::Transparent
    ...
    $txtCurrentIMOD = New-Object System.Windows.Forms.TextBox
    $txtCurrentIMOD.Name = "USBCurrentIMOD"
    $txtCurrentIMOD.ReadOnly = $true
    $txtCurrentIMOD.Width = 72
    $txtCurrentIMOD.MaxLength = 8
    $txtCurrentIMOD.Left = $lblIMOD.Right + 6
    $txtCurrentIMOD.Top = 5
    $txtCurrentIMOD.Font = $lblIMOD.Font
    $txtCurrentIMOD.BackColor = [System.Drawing.Color]::FromArgb(30, 30, 30)
    $txtCurrentIMOD.ForeColor = [System.Drawing.Color]::FromArgb(219, 219, 219)
    $txtCurrentIMOD.BorderStyle = 'FixedSingle'
    $imodPanel.Controls.Add($txtCurrentIMOD)

    $lblNewImod = New-Object System.Windows.Forms.Label
    $lblNewImod.Text = "NEW:"
    $lblNewImod.AutoSize = $true
    $lblNewImod.Left = $txtCurrentIMOD.Right + 10
    $lblNewImod.Top = 8
    $lblNewImod.Font = $lblIMOD.Font
    $lblNewImod.ForeColor = [System.Drawing.Color]::FromArgb(219, 219, 219)
    $imodPanel.Controls.Add($lblNewImod)

    $txtNewIMOD = New-Object System.Windows.Forms.TextBox
    $txtNewIMOD.Width = 70
    $txtNewIMOD.MaxLength = 6
    $txtNewIMOD.Left = $lblNewImod.Right + 6
    $txtNewIMOD.Top = 5
...
$btnReadIMOD = New-Object System.Windows.Forms.Button
$btnReadIMOD.Text = "READ"
$btnReadIMOD.Width = 58
$btnReadIMOD.Height = 28
$btnReadIMOD.Left = $lblIMODns.Right + 12
$btnReadIMOD.Top = 5
$btnReadIMOD.Tag = $device
$btnReadIMOD.BackColor = [System.Drawing.Color]::FromArgb(0, 0, 0)
$btnReadIMOD.ForeColor = [System.Drawing.Color]::FromArgb(255, 255, 255)
$btnReadIMOD.FlatStyle = [System.Windows.Forms.FlatStyle]::Flat
$btnReadIMOD.Font = New-Object System.Drawing.Font($fontCollection.Families[0], 12)
$btnReadIMOD.FlatAppearance.BorderColor = [System.Drawing.Color]::FromArgb(255, 100, 45)
$btnReadIMOD.FlatAppearance.BorderSize = 1
$imodPanel.Controls.Add($btnReadIMOD)

$btnSetIMOD = New-Object System.Windows.Forms.Button
$btnSetIMOD.Text = "SET"
$btnSetIMOD.Width = 60
$btnSetIMOD.Height = 28
$btnSetIMOD.Left = $btnReadIMOD.Right + 8
$btnSetIMOD.Top = 5
...
$btnSave.Width = 65
$btnSave.Height = 28
$btnSave.Left = $btnSetIMOD.Right + 8
$btnSave.Top = 5
...
'@
    ...
    if ($script:IsHeadlessConsole) {
        Write-Host ''
        Write-Host '  IMOD startup script saved (Startup folder, runs each logon):' -ForegroundColor DarkGray
...
$btnSetIMOD.Add_MouseLeave({
    ...
    function Read-AndDisplayIMODLocal($dev) {
        $ctrls = $deviceControls[$dev]
        $instanceId = Split-Path -Leaf $dev.RegistryPath
        $controllers = Get-WmiObject Win32_USBController | Where-Object { $_.ConfigManagerErrorCode -ne 22 }
        $matchedController = $null
        foreach ($controller in $controllers) {
            $controllerId = $controller.DeviceID -replace '\\\\', '\\'
            if ($controllerId -match [regex]::Escape($instanceId)) { $matchedController = $controller; break }
        }
    ...
    Read-AndDisplayIMODLocal $device

    $btnReadIMOD.Add_Click({
        Read-AndDisplayIMODLocal $this.Tag
    })
...
$btnSetIMOD.Add_Click({
    ...
    if (-not ($newIMOD -match '^0x[0-9A-Fa-f]{1,4}$')) {
        if ($script:IsHeadlessConsole) {
            Write-Host 'Invalid IMOD format. Use hex (e.g., 0x4E20).' -ForegroundColor Red
        } else {
    ...
    if (-not $matchedController) {
        if ($script:IsHeadlessConsole) {
            Write-Host 'No matching USB controller found for IMOD.' -ForegroundColor Red
        } else {
    ...
    if ($result) {
        ...
        if ($script:IsHeadlessConsole) {
            Write-Host 'Failed to apply IMOD settings.' -ForegroundColor Red
        } else {
...
$btnApply.Add_Click({
    ...
    if ($script:IsHeadlessConsole) {
        Write-Host 'Settings applied. A system restart may be required.' -ForegroundColor Green
    } else {
...
$btnAutoOpt.Add_Click({
    if ($script:IsHeadlessConsole) {
        ...
        $global:WarningPreference = 'SilentlyContinue'
        $global:ProgressPreference = 'SilentlyContinue'
        $script:_awxWhBk = ${function:Write-Host}
        ${function:Write-Host} = {}
    }
    try {
        ...
        function Write-ConfigFileEntriesLocal {
            ...
            if ($script:IsHeadlessConsole) {
                Write-Host "[AutoOpt] Wrote config entries to $path" -ForegroundColor Cyan
            } else {
        ...
        if ($script:IsHeadlessConsole) {
            ...
            Write-Host "[AutoOpt] Error: $_" -ForegroundColor DarkGray
        ...
        if ($script:IsHeadlessConsole -and $script:_awxWhBk) {
            ${function:Write-Host} = $script:_awxWhBk
        }
    }
})

function Get-SystemReservedCpuCoreIndices {
    ...
    $indices = @()
    for ($i = 0; $i -lt $reserved.Length; $i++) {
        if ($reserved[$i]) { $indices += $i }
    }
    return $indices
}

function Convert-MaskHexToCoreList {
    param([string]$maskHex)
    if (-not $maskHex) { return @() }
    try {
        $normalized = $maskHex.Trim()
        if ($normalized.StartsWith("0x")) { $normalized = $normalized.Substring(2) }
        $mask = [Convert]::ToUInt64($normalized, 16)
        $cores = @()
        for ($i = 0; $i -lt 64; $i++) {
            if (($mask -band ([UInt64]1 -shl $i)) -ne 0) { $cores += $i }
        }
        return $cores
    } catch {
        return @()
...
function Get-Awx2DeviceSortKey {
    param($d)
    $c = $d.Category
    $r = $d.Role
    if ($c -eq 'PCI' -and $r -eq 'GPU') { return '01-GPU' }
    if ($c -eq 'Network') { return '02-NET' }
    if ($c -eq 'PCI' -and $r -eq 'Audio') { return '03-AUDIO' }
    if ($c -eq 'USB') { return '04-USB' }
    if ($c -eq 'SSD') { return '05-SSD' }
    return ('99-{0}' -f $c)
}

function Get-Awx2DevicePresentation {
    param($d)
    $c = $d.Category
    $r = if ($d.Role) { $d.Role } else { '' }
    if ($c -eq 'Network') {
        return @{ Tag = 'NETWORK'; Accent = [ConsoleColor]::Green; Hint = 'RSS, interrupt affinity, MSI policy' }
    }
    if ($c -eq 'USB') {
        return @{ Tag = 'USB'; Accent = [ConsoleColor]::Yellow; Hint = 'xHCI host & composite endpoints' }
    }
    if ($c -eq 'SSD') {
        return @{ Tag = 'STORAGE'; Accent = [ConsoleColor]::White; Hint = 'NVMe / SATA controller' }
    }
    if ($c -eq 'PCI' -and $r -eq 'GPU') {
        return @{ Tag = 'GPU'; Accent = [ConsoleColor]::Magenta; Hint = 'Display adapter & frame pacing' }
    }
    if ($c -eq 'PCI' -and $r -eq 'Audio') {
        return @{ Tag = 'AUDIO'; Accent = [ConsoleColor]::Cyan; Hint = 'HD Audio / bus device' }
    }
    if ($c -eq 'PCI') {
        return @{ Tag = 'PCI'; Accent = [ConsoleColor]::DarkGray; Hint = 'PCIe function' }
    }
    return @{ Tag = ($c.ToUpper()); Accent = [ConsoleColor]::White; Hint = '' }
}

function Write-ConsoleAutoOptSummary {
    try {
        if (-not $script:IsHeadlessConsole) { return }
        $irqCounts = Get-DeviceIRQCounts
        $logicalCount = [Environment]::ProcessorCount
        if ($logicalCount -lt 1) { $logicalCount = 1 }
        $raw = @($deviceList | Where-Object { $_.RegistryPath -and $_.RegistryPath -ne 'Not Found' })
        $targets = $raw | Sort-Object { Get-Awx2DeviceSortKey $_ }, DisplayName
        $reservedIdx = @(Get-SystemReservedCpuCoreIndices)
        $reservedText = if ($reservedIdx.Count -gt 0) { ($reservedIdx -join ', ') } else { 'none (OS default)' }

        $sep = '  --------------------------------------------------------------------------'
        Write-Host $sep -ForegroundColor DarkGray

        foreach ($d in $targets) {
            $pr = Get-Awx2DevicePresentation $d
            Write-Host ''
            Write-Host '  [' -NoNewline -ForegroundColor DarkGray
            Write-Host $pr.Tag -NoNewline -ForegroundColor $pr.Accent
            Write-Host '] ' -NoNewline -ForegroundColor DarkGray
            Write-Host $d.DisplayName -ForegroundColor White
            if ($pr.Hint) {
                Write-Host ('      ' + $pr.Hint) -ForegroundColor DarkGray
            }
            if ($d.Role) {
                Write-Host '      Role            ' -NoNewline -ForegroundColor DarkGray
                $roleCol = if ($d.Category -eq 'SSD') { [ConsoleColor]::Blue } else { [ConsoleColor]::DarkCyan }
                Write-Host $d.Role -ForegroundColor $roleCol
            }
            $affinityPath = if ($d.Category -eq 'Network') { Get-NetworkAdapterAffinityRegistryPath $d } else { $d.RegistryPath }
            $maskHex = Get-CurrentAffinity $affinityPath
            $cores = Convert-MaskHexToCoreList $maskHex
            $setCoresText = if ($cores.Count -gt 0) { ($cores -join ', ') } else { 'Auto / system' }
            $irq = 0
            if ($d.PNPID) {
                $key = Get-PNPId $d.PNPID
                if ($irqCounts.ContainsKey($key)) { $irq = [int]$irqCounts[$key] }
            }
            Write-Host '      Affinity cores  ' -NoNewline -ForegroundColor DarkGray
            Write-Host $setCoresText -ForegroundColor Yellow
            Write-Host '      IRQ allocations ' -NoNewline -ForegroundColor DarkGray
            Write-Host $irq -ForegroundColor Yellow
            Write-Host '      Reserved CPU set' -NoNewline -ForegroundColor DarkGray
            Write-Host (' ' + $reservedText) -ForegroundColor Yellow
        }
        Write-Host ''
        Write-Host $sep -ForegroundColor DarkGray
        Write-Host '  Startup persistence: Set IMOD writes ApplyIMOD.ps1 to your user Startup folder (runs each logon).' -ForegroundColor DarkGray
        Write-Host '  Optimization pass finished. Reboot if Windows reports driver changes.' -ForegroundColor Green
        Write-Host ''
    } catch {
    }
}
